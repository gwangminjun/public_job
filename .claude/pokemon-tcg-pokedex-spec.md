...
State    : Zustand (전역) + TanStack Query (서버 상태/캐시)
...
│
├── lib/
│   ├── pokemon-api.ts            # API 클라이언트 함수
│   └── types.ts                  # TypeScript 타입 정의
│
├── components/
│   ├── sets/
│   │   ├── SetGrid.tsx           # 세트 카드 그리드
│   │   └── SetCard.tsx           # 세트 카드 컴포넌트
│   ├── cards/
│   │   ├── CardGrid.tsx          # 카드 그리드
│   │   ├── CardItem.tsx          # 카드 아이템 컴포넌트
│   │   └── CardDetail.tsx        # 카드 상세 컴포넌트
│   ├── search/
│   │   ├── SearchBar.tsx         # 검색 입력
│   │   └── FilterPanel.tsx       # 희귀도/타입 필터
│   └── common/
...
export async function GET(
  ...
  const res = await fetch(`${BASE}/sets/${params.setId}`, {
    headers: { 'X-Api-Key': API_KEY },
...
export async function GET(
  ...
  const res = await fetch(`${BASE}/cards/${params.cardId}`, {
    headers: { 'X-Api-Key': API_KEY },
    next: { revalidate: 600 },
  });
...
```
Phase 1 — 프로젝트 초기 설정
  1. Next.js 14 프로젝트 생성 (TypeScript + Tailwind)
  2. 의존성 설치 (TanStack Query, Zustand)
  3. .env.local 설정

...
#### 3-1. 관리자 인증
- **현재**: `/grandma/admin`이 누구나 접근 가능
- **개선**: 비밀번호 입력 또는 Supabase Auth 로그인 후 접근
- **방법**: `middleware.ts`에서 `/grandma/admin` 경로 인증 체크

#### 3-2. 방명록 스팸 방지
- 동일 IP 단시간 다수 작성 제한 (rate limiting)
- 욕설 필터링 (클라이언트 단어 목록 체크)
- reCAPTCHA 연동 (선택)
